...
public class ANIntergroupExchange implements AtomsetIteratorBasisDependent {
    ...
    public int size() {
    	int particles0 = moleculePair.atom0.getChildList().size()/P;
    	int particles1 = moleculePair.atom1.getChildList().size()/P;
    ...
    public List<IAtom> next() {
    	...
    	int size0 = moleculePair.atom0.getChildList().size();
    	int size1 = moleculePair.atom1.getChildList().size();
    	...
    	if (stillOnPolymer0) {
    		...
    		atoms.addAll(pair);
    		atoms.add(moleculePair.atom1.getChildList().get(counter));
    		counter++;
    		...
    		if (pair == null) {
    			...
    			if (counter == size1) {
    				...
    				if (moleculePair.atom1.getChildList().size()==P){
    		...
    		atoms.add(moleculePair.atom0.getChildList().get(counter));
    		atoms.addAll(pair);
    		counter++;
    ...
    public void reset() {
        counter = 0;
        stillOnPolymer0 = moleculePair.atom0.getChildList().size()>P;
