    ...
    public IData getData() {
        ...
        for (int i=0; i<otherDensities.length; i++) {
            ...
            for (int k=0; k<numP; k++) {
                double fac = Math.exp((p[i][k]*(vi-v0) + (uLatRhoi-uLatRho0))/(temperature*(numAtoms*D-D))) * Math.pow(v0/vi, 1.0/(numAtoms*D-D));
//                System.out.println("fac["+i+"]: "+(1-fac)/(vi-v0));
                if (fac > 2) continue;
...
//                System.out.println(p[i][k]+" "+u+" "+uLatRho0+" "+otherU+" "+uLatRhoi+" "+a0+" "+ai);
                x[i*numP+k] = Math.exp(a0-ai);
//                System.out.println(Debug.stepCount+" => "+otherDensities[i]+" Pres: "+(-temperature*(ai-a0)/(vi - v0)));
//                if (i==0 && k==(numP-1)/2) {
//                    try {
//                        fw.write(x[i*numP+k]+"\n");
//                    }
