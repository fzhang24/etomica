        ...
        switch(nBody) {
            case 1:
                if (f[0].squared() > 2.5e9) {
                    ...
                    integratorAgentManager.getAgent(atoms.get(0)).force().PEa1Tv1(-scale, f[0]);
                    if (potential == wallPotential) {
                        wallForceSum += scale*f[0].getX(wallPotential.getWallDim());
                    }
                }
                else {
                    integratorAgentManager.getAgent(atoms.get(0)).force().ME(f[0]);
                    if (potential == wallPotential) {
            ...
            case 2:
                if (f[0].squared() > 2.5e9) {
                    double scale = 50000/Math.sqrt(f[0].squared());
                    integratorAgentManager.getAgent(atoms.get(0)).force().PEa1Tv1(-scale, f[0]);
                    integratorAgentManager.getAgent(atoms.get(1)).force().PEa1Tv1(-scale, f[1]);
                }
                else {
                    integratorAgentManager.getAgent(atoms.get(0)).force().ME(f[0]);
                    integratorAgentManager.getAgent(atoms.get(1)).force().ME(f[1]);
                }
                break;
            default:
                // we hit this for bonding potentials
                for (int i = 0; i<atoms.size(); i++) {
                    integratorAgentManager.getAgent(atoms.get(i)).force().ME(f[i]);
                }
        }
    }
