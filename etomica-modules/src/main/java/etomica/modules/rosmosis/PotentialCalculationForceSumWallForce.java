        ...
        if (potential instanceof PotentialSoft) {
            ...
            switch(nBody) {
                case 1:
                    ...
                    integratorAgentManager.getAgent(atoms.get(0)).force().ME(gradient[0]);
                    break;
                case 2:
                    integratorAgentManager.getAgent(atoms.get(0)).force().ME(gradient[0]);
                    integratorAgentManager.getAgent(atoms.get(1)).force().ME(gradient[1]);
                    break;
                default:
                    ...
                    for (int i = 0; i<atoms.size(); i++) {
                        integratorAgentManager.getAgent(atoms.get(i)).force().ME(gradient[i]);
                    }
            }
        }
    }
